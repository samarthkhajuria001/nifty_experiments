...
for trading_date, day_data in df_5min.groupby('trading_date'):
    ...
    first_bar = first_30_min.iloc[0]
    rest_of_day = day_data[day_data['time'] > first_30_min.iloc[2]['time']]  # After first 3 bars
...
bull_first3_bull = bull_trend_days[bull_trend_days['first_3_all_bull'] == True]
if len(bull_first3_bull) > 0:
    closes_bull_pct = len(bull_first3_bull[bull_first3_bull['day_close_bull'] == True]) / len(bull_first3_bull) * 100
    closes_bear_pct = len(bull_first3_bull[bull_first3_bull['day_close_bull'] == False]) / len(bull_first3_bull) * 100
    low_holds_pct = len(bull_first3_bull[bull_first3_bull['first_bull_low_is_lowest'] == True]) / len(bull_first3_bull) * 100
    table_data.append(['BULL', '3 Bull Bars', f'{closes_bull_pct:.2f}%', f'{closes_bear_pct:.2f}%', f'{low_holds_pct:.2f}%', '-'])

bull_first3_bear = bull_trend_days[bull_trend_days['first_3_all_bear'] == True]
if len(bull_first3_bear) > 0:
    closes_bull_pct = len(bull_first3_bear[bull_first3_bear['day_close_bull'] == True]) / len(bull_first3_bear) * 100
    closes_bear_pct = len(bull_first3_bear[bull_first3_bear['day_close_bull'] == False]) / len(bull_first3_bear) * 100
    high_holds_pct = len(bull_first3_bear[bull_first3_bear['first_bear_high_is_highest'] == True]) / len(bull_first3_bear) * 100
    table_data.append(['BULL', '3 Bear Bars', f'{closes_bull_pct:.2f}%', f'{closes_bear_pct:.2f}%', '-', f'{high_holds_pct:.2f}%'])

# BEAR TREND
bear_trend_days = results_df[results_df['2hr_bear_trend'] == True]

bear_first3_bull = bear_trend_days[bear_trend_days['first_3_all_bull'] == True]
if len(bear_first3_bull) > 0:
    closes_bull_pct = len(bear_first3_bull[bear_first3_bull['day_close_bull'] == True]) / len(bear_first3_bull) * 100
    closes_bear_pct = len(bear_first3_bull[bear_first3_bull['day_close_bull'] == False]) / len(bear_first3_bull) * 100
    low_holds_pct = len(bear_first3_bull[bear_first3_bull['first_bull_low_is_lowest'] == True]) / len(bear_first3_bull) * 100
    table_data.append(['BEAR', '3 Bull Bars', f'{closes_bull_pct:.2f}%', f'{closes_bear_pct:.2f}%', f'{low_holds_pct:.2f}%', '-'])

bear_first3_bear = bear_trend_days[bear_trend_days['first_3_all_bear'] == True]
if len(bear_first3_bear) > 0:
    closes_bull_pct = len(bear_first3_bear[bear_first3_bear['day_close_bull'] == True]) / len(bear_first3_bear) * 100
    closes_bear_pct = len(bear_first3_bear[bear_first3_bear['day_close_bull'] == False]) / len(bear_first3_bear) * 100
    high_holds_pct = len(bear_first3_bear[bear_first3_bear['first_bear_high_is_highest'] == True]) / len(bear_first3_bear) * 100
    table_data.append(['BEAR', '3 Bear Bars', f'{closes_bull_pct:.2f}%', f'{closes_bear_pct:.2f}%', '-', f'{high_holds_pct:.2f}%'])
...
print("=" * 120)
print()
print(f"{'2HR Trend':<12} {'First 30min':<15} {'Day Close BULL':<16} {'Day Close BEAR':<16} {'1st Bull Low Holds':<20} {'1st Bear High Holds':<20}")
print("-" * 120)
...
# Save to MD file
md_content = """# Opening Pattern Analysis by 2HR Trend
...
with open('opening_pattern_by_trend.md', 'w') as f:
    f.write(md_content)

print("\nSaved to opening_pattern_by_trend.md")
